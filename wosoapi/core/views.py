...
class ClubSeason(ListView):
    ...
    def get_queryset(self):
        ...
        queryset=ClubSeasonStat.objects.select_related("club", "season", "league").filter(league=self.league)
        return queryset.order_by('league_position','season__season', 'club__name')
    def get_context_data(self, **kwargs):
        context= super().get_context_data(**kwargs)

        base_qs = (
            ClubSeasonStat.objects
            .select_related("club", "season", "league")
            .filter(league=self.league)
        )
        distinct_qs=PlayerSeasonStats.objects.select_related("club", "season", "player").filter(league=self.league)
        ...
        context['attempted_passes'] = group_by_season(base_qs, '-attempted_passes_against')
        context['completed_passes'] = group_by_season(base_qs, '-comp_passes_allowed')
        context['passes_into_final_third'] = group_by_season(base_qs, '-passes_to_final_third_allowed')
        context['passes_into_pen_area'] = group_by_season(base_qs, '-passes_to_pen_area_allowed')
        ...
        context['player_games_completed']= group_by_distinct(distinct_qs, '-matches_completed')
        context['player_matches_played']= group_by_distinct(distinct_qs, '-matches_played')
        context['player_minutes_played']= group_by_distinct(distinct_qs, '-minutes_played')
        context['player_matches_substituted']= group_by_distinct(distinct_qs, '-matches_substituted')
        ...
        context['player_xg_performance']= group_by_distinct(distinct_qs, '-xg_performance')
        context['player_npxg_performance']= group_by_distinct(distinct_qs, '-npxg_performance')
        context['player_prog_carries']= group_by_distinct(distinct_qs, '-prog_carries')
        context['player_prog_carries_final_3rd']= group_by_distinct(distinct_qs, '-prog_carries_final_3rd')
        ...
        context['player_passes_to_final_3rd']= group_by_distinct(distinct_qs, '-passes_to_final_3rd')
        context['player_passes_to_pen_area']= group_by_distinct(distinct_qs, '-passes_to_pen_area')
        context['player_pass_switches']= group_by_distinct(distinct_qs, '-pass_switches')
        context['player_through_ball']= group_by_distinct(distinct_qs, '-through_ball')
        context['player_shots_creation_action']= group_by_distinct(distinct_qs, '-shots_creation_action')
        ...
        context['player_ball_recoveries']= group_by_distinct(distinct_qs, '-ball_recoveries')
        context['player_aerial_duels_won']= group_by_distinct(distinct_qs, '-aerial_duels_won')
        context['player_aerial_duels_lost']= group_by_distinct(distinct_qs, '-aerial_duels_lost')
        ...
        context['player_interceptions']= group_by_distinct(distinct_qs, '-interceptions')
        ...
        context['player_fouls_committed']= group_by_distinct(distinct_qs, '-fouls_committed')
        context['player_carries_to_final_3rd']= group_by_distinct(distinct_qs, '-carries_to_final_3rd')
        context['player_carries_to_pen_area']= group_by_distinct(distinct_qs, '-carries_to_pen_area')
...
class ClubSeasonStatView(ListView):
    ...
    def get_queryset(self):
        ...
        queryset= ClubSeasonStat.objects.select_related("club", "season", "league").filter(club=self.club)
        return queryset.order_by('season__season', 'club__name')
    def get_context_data(self, **kwargs):     
        context= super().get_context_data(**kwargs)
        player_qs= PlayerSeasonStats.objects.select_related("club", "season", "player").filter(club=self.club)
        goalkeeper_qs = Goalkeeper.objects.select_related("player", "club", "season").filter(club=self.club)
        ...
        context['goalkeeper_season']= group_by_season(goalkeeper_qs, 'player__full_name')
...
class PlayerSeasonstatbyLeague(ListView):
    ...
    def get_context_data(self, **kwargs):
        context= super().get_context_data(**kwargs)
        context['goalkeeper']= Goalkeeper.objects.select_related("player", "club", "season").filter(league=self.league)
        return context
...
class PlayerSeasonStatbyClub(ListView):
    ...
    def get_queryset(self):
        club_name=self.kwargs.get('club_name')
        self.club= get_object_or_404(Club, name=club_name)
        queryset= PlayerSeasonStats.objects.select_related("player", "club", "season").filter(club=self.club)
        return queryset.order_by('-goals', '-assists', 'player__full_name')
    def get_context_data(self, **kwargs):
        context= super().get_context_data(**kwargs)
        context['goalkeeper']= Goalkeeper.objects.select_related("player", "club", "season").filter(club=self.club)
        return context
