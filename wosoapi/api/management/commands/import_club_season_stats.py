            ...
            for _, row in df.iterrows():
                try:
                    ...
                    stats_data = {
                        ...
                        "passes_to_final_third_allowed": row.get(
                            "passes_into_final_third"
                        ),
                        "passes_to_pen_area_allowed": row.get(
                    ...
                    obj, created = ClubSeasonStat.objects.update_or_create(
                        club=club,
                        season=season,
                        defaults={"league": league, **stats_data},
                    )
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Club with fbref_id {row['team_id']} does not exist"
                        )
                    )
...
# python manage.py import_club_season_stats SerieA_2025_26_TEAM_STATS.csv --season_id=8 --league_id=7
